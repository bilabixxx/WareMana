...
<template>
  <IconBtn id="notification-btn">
    ...
    <VMenu activator="parent" width="380px" :location="props.location" offset="21px" :close-on-content-click="false">
      <VCard class="d-flex flex-column">
        ...
        <VCardItem class="notification-section">
          ...
          <template #append>
            ...
            <IconBtn v-show="props.notifications.length" size="34" @click="markAllReadOrUnread">
              <VIcon size="20" color="high-emphasis" :icon="!isAllMarkRead ? 'bx-envelope' : 'bx-envelope-open'" />

              <VTooltip activator="parent" location="start">
                {{ !isAllMarkRead ? 'Segna come non lette' : 'Segna tutto come letto' }}
              </VTooltip>
            </IconBtn>
          </template>
        </VCardItem>
                ...
                <div class="d-flex gap-x-3">
                  ...
                  <div class="d-flex justify-space-between flex-grow-1">
                    <div>
                      ...
                      <p class="text-caption mb-0" style=" letter-spacing: 0.4px !important; line-height: 18px;">
                        {{ notification.time }}
                      </p>
                    </div>
                    <div class="d-flex flex-column gap-2">
                      <VIcon size="10" icon="bx-bxs-circle" :color="!notification.isSeen ? 'primary' : '#a8aaae'"
                        :class="`${notification.isSeen ? 'visible-in-hover' : ''}`" class="align-self-end"
                        @click.stop="toggleReadUnread(notification.isSeen, notification.id)" />
                      <VIcon size="20" icon="bx-x" class="visible-in-hover" @click="$emit('remove', notification.id)" />
                    </div>
                  </div>
                </div>
              </VListItem>
        ...
        <!-- 👉 Footer -->
        <VCardText v-show="props.notifications.length" class="py-4 px-5">
