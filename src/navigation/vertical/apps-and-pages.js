/* Mapping delle sottocategorie 
const categoryRowsMapping = (row) => {
  return {
      title: row.name,
      to: { name: 'apps-ecommerce-product-categories-id', params: { id: row.id } },
  };
}

/* Mapping delle categorie di primo Livello 
const categoryMapping = (category) => {
  return {
      title: category.name,
      to: { name: 'apps-ecommerce-product-categories-id', params: { id: category.id } },
      children: category.rows.map(categoryRowsMapping)
  };
};

/* fetching delle Categorie 
const fetchAndProcessCategories = async () => {
  const { data: categories } = await useApi(createUrl('/v1/categorie/menu', {
      query: { levelId: 1 },
  }));
  return categories.value.map(categoryMapping);
}

const categoriesList = await fetchAndProcessCategories();*/
export default [
  { heading: 'Apps & Pages' },
  {
    title: 'Ecommerce',
    ...
    children: [
      ...
      },
      {
        title: 'Prodotti',
        children: [
          { title: 'Tutti i Prodotti', to: 'apps-ecommerce-product-list' },
