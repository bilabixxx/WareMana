<script setup>
import { auth, db } from '@/firebase/config'; // Importa Firestore dalla tua configurazione esistente
...
const login = async () => {
  ...
  try {
    ...
    const adminDocRef = doc(db, "employees", userId);
    ...
    if (adminDoc.exists()) {
      ...
      console.log("Esiste", adminDoc.data());

      useCookie('userAbilityRules').value = userAbilityRules;
      ability.update(userAbilityRules);

      useCookie('userData').value = { uid: userId, email: user.email, role: 'client' };

      await nextTick(() => {
        router.replace(route.query.to ? String(route.query.to) : '/');
      });
    } else {
      throw { status: 401, message: "Unauthorized: You are not an employee" };
    }
  } catch (err) {
    if (err.status === 401) {
